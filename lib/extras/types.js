...
export default function builtin(ctx, declareFn) {
  declareFn(ctx, {
    name: 'type',
    pure: true,
    fn: function typeFn(referenceTypeBlock) {
      const typeContext = Object.create(null);
      declareFn(typeContext, {
        name: 'check',
        pure: true,
        fn: function check(value) {
          return typeCheck(typeCheckContext, value, referenceTypeBlock);
        },
      });
      declareFn(typeContext, {
        name: 'match',
        pure: true,
        fn: function match(targetType) {
          return typeMatch(
            typeToMatchContext,
            targetType,
            typeMathersContext,
  ...
  // TODO `.block-of ?string` to check if a block has values of a given type(s). like `dict-param: type { .block-of [ ?string ?any ] , % 2 }` ???
  // TODO `.block-in context` to check if a block can be executed in a given context (ie the context provide all names required by the block) how to get the context?
  // TODO `.fn [ ?string ?number ] ?string` to check if a value is a function
  ...
  declareFn(typeCheckContext, {
    name: 'dict',
    pure: true,
    fn: function dict(value, valueType) {
      if (!value || typeof value !== 'object' || Array.isArray(value)) {
        return false;
      }
      if (value instanceof Map) {
        return Array.from(value.values()).every((v) =>
          runType(valueType, typeCheckContext, v),
        );
      }
...
function typeCheck(typeCheckContext, value, referenceTypeBlock) {
  const checks = listWithValue.call(
    typeCheckContext,
    referenceTypeBlock,
    value,
  );
...
function typeMatch(
  targetTypeMatchContext,
  targetTypeBlock,
  referenceTypeMatchContext,
  referenceTypeBlock,
) {
