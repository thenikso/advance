...
export function run(
  ...
  let node, nextNode, value;
  for (let i = offset, l = expression.length; i < l; i++) {
    if (
      ...
      switch (node.kind) {
        ...
        case 'pipe':
          ...
          if (typeof value === 'undefined') {
            throw new Error(`word not found: ${node.toString()}`);
          }
          if (cursorExpectCount > 0) {
        ...
        case 'cpath':
          ...
          for (let i = 0, l = node.path.length; i < l; i++) {
            ...
            if (typeof value === 'undefined') {
              throw new Error(`word not found: ${node.path[i]}`);
            }
          }
          if (typeof value === 'function') {
            if (value.length > 0) {
    ...
    if (isList(node)) {
      ...
      for (let i = 0, l = node.length, v; i < l; ) {
        v = run(/** @type {any} */ (node), context, undefined, i, true);
        i = v.index;
        value.push(v.value);
      }
      provide(value);
  ...
  if (cursorExpectCount > 0) {
    // TODO print incomplete expression using cursorQueue
